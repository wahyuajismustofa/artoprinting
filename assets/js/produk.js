...
function initDOMReferences() {
    produkSelect = document.getElementById('produk');
    kategoriSelect = document.getElementById('kategori');
    produkListSection = document.getElementById('produk-list');
...
function renderProdukSelect() {
    ...
    produkSelect.innerHTML = produkList.map(p =>
        `<option value="${p}"${p === 'Undangan_Cetak' ? ' selected' : ''}>${p.replace(/_/g, ' ')}</option>`
    ).join('');
}
